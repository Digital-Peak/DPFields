...
class DPFieldsController extends JControllerLegacy
{
	...
	public function display ($cachable = false, $urlparams = false)
	{
		...
		// Check for edit form.
		if ($vName == 'field' && $lName == 'edit' && ! $this->checkEditId('com_dpfields.edit.field', $id))
		{
			// Somehow the person just went to the form - we don't allow that.
			$this->setError(JText::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $id));
			$this->setMessage($this->getError(), 'error');
			$this->setRedirect(JRoute::_('index.php?option=com_dpfields&view=fields&context=' . $this->context, false));
		...
		if ($view = $this->getView($vName, $vFormat))
		{
			...
			$model = $this->getModel($vName, 'DPFieldsModel', array(
