	...
	protected function getInput ()
	{
		...
		if ($allowClear && ! $scriptClear)
		{
			...
			$script[] = '		document.getElementById(id + "_name").value = "' .
					 htmlspecialchars(JText::_('COM_DPFIELDS_SELECT_A_FIELD', true), ENT_COMPAT, 'UTF-8') . '";';
		...
		$html = array();
		$link = 'index.php?option=com_dpfields&amp;view=fields&amp;layout=modal&amp;tmpl=component&amp;context=' . $context .
		...
		if ((int) $this->value > 0)
		{
			...
			$query = $db->getQuery(true)
				...
				->from($db->quoteName('#__dpfields_fields'))
		...
		if (empty($title))
		{
			$title = JText::_('COM_DPFIELDS_SELECT_A_FIELD');
		}
		...
		$html[] = '<a href="#modalCategory-' . $this->id . '" class="btn hasTooltip" role="button"  data-toggle="modal"' . ' title="' .
				 JHtml::tooltipText('COM_DPFIELDS_CHANGE_FIELD') . '">' . '<span class="icon-file"></span> ' . JText::_('JSELECT') . '</a>';

		// Edit field button
		if ($allowEdit)
		{
			$html[] = '<a' . ' class="btn hasTooltip' . ($value ? '' : ' hidden') . '"' .
					 ' href="index.php?option=com_dpfields&layout=modal&tmpl=component&task=field.edit&id=' . $value . '"' . ' target="_blank"' .
					 ' title="' . JHtml::tooltipText('COM_DPFIELDS_EDIT_FIELD') . '" >' . '<span class="icon-edit"></span>' . JText::_('JACTION_EDIT') .
					 '</a>';
