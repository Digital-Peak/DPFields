...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_dpfields&task=fields.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'fieldList', 'adminForm', strtolower($listDirn), $saveOrderingUrl, false, true);
}
?>

<form action="<?php echo JRoute::_('index.php?option=com_dpfields&view=fields'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<?php else : ?>
						...
						<tr class="row<?php echo $i % 2; ?>" item-id="<?php echo $item->id ?>">
							...
							<td>
								...
								<?php if ($canEdit || $canEditOwn) : ?>
									<a href="<?php echo JRoute::_('index.php?option=com_dpfields&task=field.edit&id=' . $item->id . '&context=' . $context); ?>">
							...
							<td class="small">
								<?php
								$label = 'COM_DPFIELDS_TYPE_' . strtoupper($item->type);
			...
			<?php if ($user->authorise('core.create', $context)
				...
				<?php echo JHtml::_(
						...
						'collapseModal',
						array(
							'title' => JText::_('COM_DPFIELDS_VIEW_FIELDS_BATCH_OPTIONS'),
